...
TEST(TtlCache, SetGetBeforeExpiry) {
    ...
    cache.set("key", "value", c.now(), 1000);
    auto v = cache.get("key", c.now());
...
TEST(TtlCache, EvictRemovesExpired) {
    ...
    cache.set("a", "1", c.now(), 10);
    cache.set("b", "2", c.now(), 50);

    c.advance(11); // "a" expired, "b" not yet
    auto removed = cache.evict(c.now(), 1000);

    EXPECT_EQ(removed, 1u);
    EXPECT_FALSE(cache.get("a", c.now()).has_value());
    EXPECT_TRUE(cache.get("b", c.now()).has_value());
}
...
TEST(TtlCache, LazyHeapInvalidationOnUpdate) {
    ...
    cache.set("k", "v1", c.now(), 100); // expiry at t=100
    // overwrite same key with longer TTL -> old heap node becomes stale
    cache.set("k", "v2", c.now(), 1000); // expiry at t=1000
...
TEST(TtlCache, SlidingPolicyExtendsExpiry) {
    ...
    cache.set("k", "v", c.now(), 50); // initially expires at 50

    c.advance(40);
    ASSERT_TRUE(cache.get("k", c.now()).has_value()); // extends expiry to 140

    c.advance(60); // now at 100, would have expired without extension
    ASSERT_TRUE(cache.get("k", c.now()).has_value());
}
...
TEST(TtlCache, ExtendOnlyNearExpiryThreshold) {
    ...
    cache.set("k", "v", c.now(), 1000); // expires at 1000
